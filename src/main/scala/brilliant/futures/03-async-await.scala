...
/** While the for comprehension syntax is a dramatic improvement from the
  ...
  * accessible than normal imperative style code. We have to learn this new for
  * comprehension syntax, which can create a barrier to usability, particularly
  * for developers from other programming languages.
  *
  ...
  * happens and the actual "work" is not done in the for comprehension. This can
  * be confusing, especially for developers used to working with a more
  * "functional" style of programming.
  *
  * The async await functionality tries to address both of these concerns. Note
  * that this is not part of the Scala standard library and needs to be added as
  * a separate module. See the build file for this project for an example of
  * this.
  */
object AsyncAwait extends App {
  ...
  /** Let's rewrite the example above to use the async await syntax. Our
    ...
    * here assigning them to the values `two` and `three`. Then instead of using
    * a for comprehension to collection the results we use the `await` operator.
    *
    ...
    * internally.
    */
  def future3Sequential(implicit ec: ExecutionContext): Future[Int] =
    async {
      await(asyncTwo) + await(asyncThree)
    }

  def future3Parallel(implicit ec: ExecutionContext): Future[Int] =
    async {
      val two = asyncTwo
      val three = asyncThree
  ...
  // def future3Prime(implicit  ec: ExecutionContext): Future[Int] = {
  //     val two = asyncTwo
  //     val three = asyncThree
  //     // Future { two.await + three.await} // not what we want to do and not what async await
  //     for {
  //       x <- two
  //       y <- three
  //     } yield x + y
...
object Practice {
  ...
  /** Constructs an event describing an update to a user and streams it out.
    */
  def streamUserUpdate(oldUser: User, newUser: User)(implicit
      ec: ExecutionContext
  ): Future[Unit] =
    ???
  ...
  /** Using the async await syntax, write a program that will load a user,
    ...
    */
  def myProgram1ForComprehension(id: Int)(implicit ec: ExecutionContext): Future[Unit] = {
    for {
      _    <- checkPermissions
      user <- getUser(id)
      updated = updateUser(user)
      _ <- saveAndStreamInParallelForComprehension(user, updated)
    } yield ()
  }

  def saveAndStreamInParallelForComprehension(oldUser: User, newUser: User)(implicit
      ec: ExecutionContext
  ): Future[Unit] = {
    val saveFuture = saveUser(newUser)
    val updateFuture = streamUserUpdate(oldUser, newUser)
    for {
      _ <- saveFuture
      _ <- updateFuture
    } yield ()
  ...
  def myProgram1AsyncAwait(id: Int)(implicit ec: ExecutionContext): Future[Unit] = {
    async {
      await(checkPermissions)
      val user = await(getUser(id))
      val updatedUser = updateUser(user)
      val saveUserFuture = saveUser(updatedUser)
      await(saveUserFuture)
      val streamUpdateFuture = streamUserUpdate(user, updatedUser)
      await(streamUpdateFuture)
    }
  }

  /** Assume we have now concluded that we can only stream out the update after
    * we have successfully saved the updated user to the database. Revise your
    * implementation to reflect this new requirement.
    */
